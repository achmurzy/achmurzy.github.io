...
function generateLine(point, width)
{
...
function generateQuadratic(point, width)
{
	var end = RandomInsideBox();
	var control_point = RandomInsideBox();
	
	var perpStart = perpindicularQuadratic([point, control_point, end], 0, true).normalize();
	...
	var perpEnd = perpindicularQuadratic([point, control_point, end], 1, true).normalize();
	perpCp.add(perpEnd);
	...
	return ['Q', spos, sneg, pos, neg, cpos, cneg, point, end, control_point, perpStart, perpEnd, perpCp];	
}
...
function generateCubic(point, width)
{
	var end = RandomInsideBox();
	...
	var startPerp = 
	perpindicularCubic([point, control_point_1, control_point_2, end], 0, true).normalize();

	spos.add(new Victor(startPerp.x * width, startPerp.y * width));
	sneg.add(new Victor(startPerp.x * -width, startPerp.y * -width));

	var endPerp = 
	perpindicularCubic([point, control_point_1, control_point_2, end], 1, true).normalize();
